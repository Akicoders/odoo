...
RUN curl -L -o /tmp/wkhtmltopdf.deb \
    https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/0.12.6.1-2/wkhtmltox_0.12.6.1-2.bullseye_amd64.deb \
...
RUN mkdir -p /var/lib/odoo /var/log/odoo && \
    chown -R odooodoo /var/lib/odoo /var/log/odoo

RUN touch /etc/odoo/odoo.conf && chown odooodoo /etc/odoo/odoo.conf
